...
async def kartu_detail(
    ...
    """Kembalikan satu objek Kartu Ekonomi Desa utuh (identitas, peta_peran,
    rekomendasi_aksi, potensi, kesiapan, biofisik, logistik, jalur_ekonomi,
    desa_kembar, fakta_program, mutu_data) apa adanya dari
    `kartu-ekonomi/kartu/<idkab>.json`.

    `iddesa` dicari lebih dulu di `simpanan.indeks_per_desa` untuk menentukan
    `idkab`-nya; tak dikenal di sana atau tak ada di berkas kartu kabupatennya
    sama-sama dianggap desa tak dikenal (404 `DESA_TIDAK_ADA`).
    """
